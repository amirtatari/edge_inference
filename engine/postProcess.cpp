...
void PostProcess::applyNms(const std::vector<cv::Rect>& boxes, 
...
*/
std::optional<modelIo::DetectedObjects> 
PostProcess::yoloFivePostProc(float* outputTensorData, int frameWidth, int frameHeight, int numBoxes, int numClasses) const
{
  ...
  {
    const float objectness_score {outputTensorData[i * (numClasses + 5) + 4]};
    if (objectness_score > m_config->m_confidenceThreshold)
    {
      const float* class_probabilities {&outputTensorData[i * (numClasses + 5) + 5]};
      int best_class_id {-1};
      ...
      {
        const float x_center {outputTensorData[i * (numClasses + 5) + 0]};
        const float y_center {outputTensorData[i * (numClasses + 5) + 1]};
        const float width {outputTensorData[i * (numClasses + 5) + 2]};
        const float height {outputTensorData[i * (numClasses + 5) + 3]};
...
*/
std::optional<modelIo::DetectedObjects> 
PostProcess::yoloEightPostProc(void* data, int frameWidth, int frameHeight, int numBoxes, int numClasses) const
{
...
*/
std::optional<modelIo::DetectedObjects> 
PostProcess::yoloTenPostProc(void* data, int frameWidth, int frameHeight, int numBoxes) const
{
  const float* outputTensorData {static_cast<const float*>(data)};
...
float PostProcess::calculateIoU(const cv::Rect& box1, const cv::Rect& box2) const
{
...
*/
std::optional<modelIo::DetectedObjects> 
PostProcess::ssdPostProc(void* data, int frameWidth, int frameHeight, int numBoxes) const
{
  const float* outputTensorData {static_cast<const float*>(data)};
...
std::optional<modelIo::DetectedSemantics> 
PostProcess::semanticPostProc(void* data, int outW, int outH, int numClasses,
